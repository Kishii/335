...
bool IsPositiveSpell(SpellEntry const *spellProto)
{
    ...
    for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
        if (!IsPositiveEffect(spellProto, SpellEffectIndex(i)))
            return false;
    return true;
}
...
bool SpellMgr::IsNoStackSpellDueToSpell(uint32 spellId_1, uint32 spellId_2) const
{
    ...
    switch(spellInfo_1->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
            switch(spellInfo_2->SpellFamilyName)
            {
                ...
                case SPELLFAMILY_WARRIOR:
                {
                    ...
                    // Stoicism (Warrior Tier-10 4-piece set bonus) and Shield Block
                    if( spellInfo_1->Id == 70845 && spellInfo_2->Id == 2565 )
                        return false;
                    break;
                }
                case SPELLFAMILY_DRUID:
        ...
        case SPELLFAMILY_WARRIOR:
            ...
            // Stoicism (Warrior Tier-10 4-piece set bonus) and Shield Block
            if( spellInfo_2->Id == 70845 && spellInfo_1->Id == 2565 )
                return false;
            break;
        case SPELLFAMILY_PRIEST:
        ...
        case SPELLFAMILY_PALADIN:
            if( spellInfo_2->SpellFamilyName == SPELLFAMILY_PALADIN )
            {
                ...
                // Repentir ( 20066 ) enlève maintenant Vengeance vertueuse ( 61840 ).
